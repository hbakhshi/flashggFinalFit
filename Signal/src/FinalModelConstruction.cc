...
void FinalModelConstruction::getRvFractionFunc(string name){
  ...
  for (unsigned int i=0; i<allMH_.size(); i++){
    int mh = allMH_[i];
    if( (proc_=="testBBH" || proc_=="thq" || proc_=="thw") && mh!=125 ) continue;
...
void FinalModelConstruction::getNormalization(){
  ...
  for (unsigned int i=0; i<allMH_.size(); i++){
    ...
    if( (proc_=="testBBH" || proc_=="thq" || proc_=="thw") ) temp->SetPoint(0,mh,effAcc);
    else temp->SetPoint(i,mh,effAcc);
    std::cout << "ED DEBUG: added point to temp graph: i, mh, effAcc = " << i << ", " << mh << ", " << effAcc << std::endl;
  }
  if( (proc_=="testBBH" || proc_=="thq" || proc_=="thw") ) temp->RemovePoint(0);
